...
export const Render: React.FC<RouteItem> = React.memo(({ mode, rule, current, define, items }) => {
  ...
  return mode === 'simple' ? (
    ...
    <Relative.Provider value={instance}>
      <Container>
        {mode === 'recursive' ? <RenderSwitch value={items} notFound={notFound} redirect={redirect} /> : <Component />}
      </Container>
    </Relative.Provider>
  );
});
...
const RenderSwitch: React.FC<RenderRouteProps> = ({ value, notFound, redirect }) => {
  return (
    <Switch>
      {value.map((it) => (
        <Route {...it.rule} component={() => <Render {...it} />} />
      ))}
