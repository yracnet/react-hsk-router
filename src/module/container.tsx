...
export const NotDefined: React.FC = () => {
  ...
  return (
    <div className="not-defined">
      ...
      <h3>
        The <b>{pathname}</b> does not defined a component
      </h3>
    </div>
  );
};
...
export const WrapperContainer: React.FC = ({ children }) => {
  ...
  return (
    <div className="p-2 border border-primary">
      {children}
      <sub>Wrapper Container: {random}</sub>
    </div>
  );
};
    ...
    <div className="m-2">
      <div className="card">
          ...
          <ul className="nav nav-pills">
            ...
            {current.views.map((it) => (
              <li key={it.path} className="nav-item">
                <NavLink to={it.uri} {...rule} className="nav-link">
                  {it.title || it.path}
                </NavLink>
              </li>
            ))}
        ...
        <div className="card-body">
          {children}
          <sub>Debug Container: {random}</sub>
        </div>
      </div>
    </div>
  );
